...
<div id="group" class="wrap wapo-plugin">
	...
	<h1>
		<?php if ( $group->id > 0 ) :
			...
			<a href="edit.php?post_type=product&page=yith_wapo_group&id=<?php echo $group->id; ?>" class="page-title-action"><?php echo __( 'Edit group', 'yith-woocommerce-product-add-ons' ); ?> &raquo;</a>
	...
	<?php if ( $group->id > 0 ) :
				...
				foreach ( $rows as $key => $value ) :
					...
					<li id="type-<?php echo $value->id; ?>" class="type-row">
						...
						<div href="#type-form-<?php echo $value->id; ?>" class="wapo-type-edit" style="cursor: pointer;">
							...
							<?php endif; ?>
							<a href="edit.php?post_type=product&page=yith_wapo_group_addons&id=<?php echo $group->id; ?>&delete_addons_id=<?php echo $value->id; ?>" class="button delete_addons" style="font-weight: normal; top: 10px; right: 10px; position: absolute;">
								<i class="dashicons dashicons-no" style="line-height: 27px;"></i>
								<?php echo __( 'Delete', 'yith-woocommerce-product-add-ons' ); ?>
							</a>
						</div>
		...
		<div id="save-addons-order">
			<form action="edit.php?post_type=product&page=yith_wapo_group_addons&id=<?php echo $group->id; ?>" method="post">
				...
				<input type="submit" class="button button-primary" value="<?php echo __( 'Save addons order', 'yith-woocommerce-product-add-ons' ); ?>">
			</form>
		</div>
